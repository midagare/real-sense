...
int main(int argc, char** argv)
{
	...
	cfg2.enable_stream(RS2_STREAM_COLOR, width2, height2, RS2_FORMAT_BGR8, fps2);
	...
	rs2::pipeline_profile pipe_profile = pipe.start(cfg2);
	...
	while (1)
	{
		...
		cv::Mat dMat_depth = cv::Mat(cv::Size(width2, height2), CV_16UC1, (void*)depth.get_data());

		std::string sensor = "_YS";
		...
		if (c == 's')
		{
			...
			save_img(std::ref(ir_frame_left), std::ref(rgb_size), RS_IR_L, CV_8UC1, datetime, sensor, 0);
			save_img(std::ref(ir_frame_right), std::ref(rgb_size), RS_IR_R, CV_8UC1, datetime, sensor, 0);
			save_img(std::ref(color_frame), std::ref(rgb_size), RS_RGB, CV_8UC3, datetime, sensor, 0);
			cv::imwrite("Ima/" + datetime + sensor + "_D_" + std::to_string(0) + ".png", dMat_depth);
		}
		else if (c == 'y')
		{
			d_sensor.set_option(RS2_OPTION_EMITTER_ENABLED, 1.f);
